...
• Falta de capacitación en cálculos y tablas de carga.
    ...
    def update_sling_tension_chart(self, load_weight, num_sling_legs):
        ...
        if num_sling_legs <= 0: 
            ...
            self.ax.set_title(f"Tensión Eslinga (Carga: {load_weight:.0f}kg, Patas: {num_sling_legs}) - Inválido", fontsize=9)
        ...
        for angle_h in angles_h:
            if 0 < angle_h < 90: # Ángulo horizontal válido
                angle_v = 90 - angle_h 
                cos_angle_v = math.cos(to_radians(angle_v))
                if cos_angle_v != 0:
                    tension = (load_weight / num_sling_legs) / cos_angle_v
        ...
        self.ax.set_title(f"Tensión Eslinga (Carga: {load_weight:.0f}kg, Patas: {num_sling_legs})", fontsize=9)
